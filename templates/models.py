...
class ExpenseManager:
    ...
    def update_person(self, person_id, new_username):
        ...
        duplicate = Person.query.filter(
            Person.username == new_username, Person.id != person.id
        ).first()
        if duplicate is not None:
            raise ValueError(f"'{new_username}' is already taken")

        person.username = new_username
        db.session.commit()

    def update_expense(self, expense_id, description, amount, paid_by, participants):
        ...
        expense.payer = payer
        expense.participant_list = participant_people
